...
- **Chips/botones con color de tienda dinámico**: el color de fondo y el de texto NO pueden venir de dos clases distintas que compiten por orden de cascada (p. ej. `bg-[var(--color-superficie)]` contra `bg-[var(--chip-color)]`: el orden en el CSS compilado lo decide, y en este proyecto la clase "base" gana y pinta el chip de blanco). Solución: las propiedades de color/fondo/borde de estado VAN en la clase de ese estado (activo/inactivo), NO en la clase base; el par fondo+contraste se define como variables CSS (`--chip-color` + `--chip-texto`) en el mismo `style` inline donde se conoce el `colorPrimario`. Antes de dar por arreglado un tema visual hay que verificarlo en el navegador real y revisar el ORDEN de todas las propiedades en el CSS compilado.
- **Texto blanco sobre color de tienda data-driven** (`colorPrimario`, `--gestion-color`, `--primario`, `--talle-color`): si el color es claro (p. ej. bebés `#F59E6B`), `text-white` queda ilegible: son "letras apagadas" por contraste, no por cascada. Regla: todo elemento cuyo fondo usa un color de tienda lleva en el MISMO `style` inline un token de contraste calculado (`--gestion-texto`, `--primario-texto`, `--talle-texto`, `--chip-texto`, vía `colorContraste()` de `src/utils/textoContraste.ts`), y el texto usa `text-[var(--...texto,<fallback>)]` (fallback `#fff` si el fondo por defecto es oscuro, `#1b1b1f` si es blanco). Nunca `text-white` duro sobre color de tienda. **Excepción (norma visual del hero de TiendaPage):** si la estructura GARANTIZA un fondo oscuro, el texto puede (y debe) ser blanco fijo para quedar uniforme entre las 4 tiendas: el hero combina dos capas de fondo (el gradiente `linear-gradient(135deg,var(--primario),var(--secundario))` más un velo oscuro encima `linear-gradient(135deg,rgba(12,12,20,0.34),rgba(12,12,20,0.64))`); con ese velo `text-white` lee bien incluso en la tienda más clara (`#F59E6B`). Igual el FAB del carrito: atenuar el color con `color-mix(in srgb, var(--primario) 65%, #1e1b2e)` + texto blanco. Fondo oscuro por estructura → `text-white` OK; fondo con el color crudo de la tienda → token de contraste.
- **Las guías del frontend (`docs/frontend/ui-guidelines.md` y reglas React globales) describen CSS Modules, pero el proyecto real usa Tailwind v4 con variables en `src/styles/global.css`.** Antes de tocar estilos, confirmar el stack real (deps, `src/styles/global.css`, clases arbitrarias) y, si corresponde, proponer actualizar esas guías para que reflejen la realidad.
- **Migraciones one-off se eliminan cuando terminan.** Un `CommandLineRunner` pensado para una migración puntual (p. ej. `ImagenMigrator` que re-subía imágenes a Cloudinary) corre en CADA arranque y ya no aporta nada una vez aplicado; verificar que los datos quedaron migrados (chequeo en BD) y borrarlo junto con los métodos que solo él usaba.
