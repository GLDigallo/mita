...
- **Chips/botones con color de tienda dinámico**: el color de fondo y el de texto NO pueden venir de dos clases distintas que compiten por orden de cascada (p. ej. `text-[var(--color-texto-suave)]` contra `text-white`). Cada estado (activo/inactivo) debe tener UNA sola regla de color; el par fondo+contraste se define como variables CSS (`--chip-color` + `--chip-texto`) en el mismo `style` inline donde se conoce el `colorPrimario`. El texto del chip activo se lee de `--chip-texto` (blanco/negro según contraste), no de clases `text-white/text-black`. Antes de dar por arreglado un tema visual de color hay que verificarlo en el navegador (con el proyecto real, no sólo leyendo el CSS compilado).
